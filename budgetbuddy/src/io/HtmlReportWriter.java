...
public class HtmlReportWriter implements Report{
    ReportCreator creator = new HtmlCreator();
...
/*public class HtmlReportWriter extends Report {
    ...
    protected void writeRecentEntriesSection(BufferedWriter writer, List<Expense> expenses) throws IOException {
        ...
        for (int i = expenses.size() - 1; i >= 0 && count < 10; i--, count++) {
            ...
            writer.write(String.format("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>\n",
                    dateStr, exp.getCategory(), formatAmount(exp.getAmount()), exp.getNotes()));
        }
        writer.write("</table>\n");
    }
}
